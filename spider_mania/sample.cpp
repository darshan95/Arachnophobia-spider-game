...
int num_spiders;
float canon_y = -box_width/2 + 0.25 - canon_width - 0.4 + 1.4;
float toggreen,togred,togcan,rotcan,mousex,mousey;
float prev_r_min,prev_g_min,prev_r_max,prev_g_max,prev_c_min,prev_c_max;
float spider_ground=0;
...
int main(int argc, char **argv) {
    ...
    // scanf("%d", &difficulty);
    prev_r_min = -box_len/2;
    prev_r_max = box_len/2;
    prev_g_max = box_len/2 ;
    prev_g_min = -box_len/2;
    prev_c_min = -box_len/2;
    prev_c_max = box_len/2;
            ...
            if(counter%3 == 0)
            {
                ...
                while(var!=NULL)
                {
                    ...
                    else
                    {
                        ...
                        if((var->spider->pos[0] > basket1_x) && (var->spider->pos[0] < prev_g_max))
                            prev_g_max = basket1_x + basket_len/2;
                        if((var->spider->pos[0] > canon_x) && (var->spider->pos[0] < prev_c_max))
                            prev_c_max = canon_x + canon_len + 0.05f;
                        if((var->spider->pos[0] < canon_x) && (var->spider->pos[0] > prev_c_min))
                            prev_c_min = canon_x - canon_len - 0.05f;
                        if((var->spider->pos[0] < basket1_x) && (var->spider->pos[0] > prev_g_min))
                            prev_g_min = basket1_x - basket_len/2;
                        if((var->spider->pos[0] < basket2_x) && (var->spider->pos[0] > prev_r_min))
                            prev_r_min = basket2_x - basket_len/2;
                        if((var->spider->pos[0] > basket2_x) && (var->spider->pos[0] < prev_r_max))
                            prev_r_max = basket2_x + basket_len/2;




                        /* if(prev_r_min > temp_spider->pos[0])
                           {
...
void handleMouseclick(int button, int state, int x, int y) 
{
    ...
    mousex=mouseposx;
    mousey=mouseposy;
    printf("x--- %d y--- %d mousex-- %f mousey--- %f width-- %d height-- %d canon--- %f canony--- %f\n",x,y,mousex,mousey,windowWidth,windowHeight,canon_x,canon_y);
    if (state == GLUT_DOWN)
    {
        if (button == GLUT_LEFT_BUTTON)
        {
            printf("%f\n", distance(mousex,mousey,basket1_x,basket_y));
            if(distance(mousex,mousey,canon_x,canon_y)<0.2)
            {
                togcan=1;
                toggreen=0;
                togred=0;
            ...
            else if(distance(mousex,mousey,basket1_x,basket_y)<0.2)
            {
                togcan=0;
                toggreen=1;
                togred=0;
            ...
            else if(distance(mousex,mousey,basket2_x,basket_y)<0.2)
            {
                togcan=0;
                toggreen=0;
                togred=1;
          ...
          //  toggreen=0;
        else if(button == GLUT_RIGHT_BUTTON)
            {
                ...
                toggreen=0;
                togred=0;
                if(distance(mousex,mousey,canon_x,canon_y)<0.5)
                {
...
{
    float  move;
    if(distance(mousex,mousey,position,canon_y)<0.7)
    {   
...
void dragwithmouse(int x,int y)
{
    ...
    {
        printf("dsaDSA");
        basket2_x=drag(basket2_x);//,prev_r_min,prev_r_max);
    }
    else if(toggreen)
    {
        printf("DSA");
        basket1_x=drag(basket1_x);//,prev_g_min,prev_g_max);
    }
    else if(togcan)
    {
        canon_x=drag(canon_x);//,prev_c_min,prev_c_max);
